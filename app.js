...
function wireFormHandlers() {
  ...
  if (templateEditSearch) {
    templateEditSearch.addEventListener('input', handleSequenceEditTemplateSearch);
  }
  const templateEditSelect = document.getElementById('sequenceStepEditTemplate');
  if (templateEditSelect) {
    templateEditSelect.addEventListener('change', () => {
      const searchInput = document.getElementById('sequenceStepEditTemplateSearch');
      const selected = templateEditSelect.options[templateEditSelect.selectedIndex];
      if (searchInput && selected) {
        searchInput.value = selected.textContent || '';
      }
    });
  }
  const cancelSequenceStepEditBtn = document.getElementById('cancelSequenceStepEditBtn');
  if (cancelSequenceStepEditBtn) {
    cancelSequenceStepEditBtn.addEventListener('click', closeSequenceStepEditModal);
  }
  ...
  const sequenceTemplateSelect = document.getElementById('sequenceStepTemplateSelect');
  ...
  const cancelSequenceStepBtn = document.getElementById('cancelSequenceStepBtn');
  if (cancelSequenceStepBtn) cancelSequenceStepBtn.addEventListener('click', closeSequenceStepModal);
...
function renderTemplateList() {
  ...
  container.innerHTML = state.templates
    .map(
      (tpl, idx) => `
        <div class="template-card" data-template-index="${idx}">
          <div class="template-card-header">
            <h4>${tpl.name || 'Untitled'}</h4>
            <button class="inline-btn primary template-edit-btn" data-template-index="${idx}">Edit</button>
          </div>
...
function openContactSequenceEnrollModal() {
  ...
  if (!modal) return;
  populateContactSequenceOptions();
  const startNow = document.getElementById('contactSequenceStartNow');
...
async function handleContactSequenceEnroll() {
  ...
  const sequence =
    state.sequences.find((s) => String(s.id) === seqName) ||
    state.sequences.find((s) => s.name === seqName);
  if (!sequence) {
...
function openSequenceStepModal() {
  ...
  // Ensure any open preview modal is hidden so this form is on top.
...
function updateSequenceStepTemplateVisibility() {
  ...
  const templateSelect = document.getElementById('sequenceStepTemplateSelect');
  if (!typeSelect || !templateLabel || !templateSelect) return;
  const type = typeSelect.value;
  const showTemplate = type === 'email';
  templateLabel.classList.toggle('hidden', !showTemplate);
  templateSelect.required = showTemplate;
  if (!showTemplate) {
...
async function handleSequenceStepFormSubmit(event) {
  ...
  const templateId = stepType === 'email' ? templateSelect.value : null;
  if (stepType === 'email' && !templateId) {
...
function openSequenceStepEdit(stepId) {
  ...
  const templateSelect = document.getElementById('sequenceStepEditTemplate');
  ...
  if (!modal || !nameInput || !delayInput || !templateSelect || !templateLabel) return;
  populateSequenceEditTemplateOptions();
  nameInput.value = step.name || '';
  delayInput.value = step.position === 1 ? 0 : step.delay_days || 0;
  const isEmail = step.step_type === 'email';
  templateLabel.classList.toggle('hidden', !isEmail);
  templateSelect.required = isEmail;
  const templateIdStr = isEmail && step.template_id ? String(step.template_id) : '';
  templateSelect.value = templateIdStr;
  if (templateSearch) {
...
function populateSequenceEditTemplateOptions() {
  ...
  const datalist = document.getElementById('sequenceStepEditTemplateOptions');
  if (!select) return;
  const options = ['<option value="">Select a template</option>'];
  const dataOpts = [];
  state.templates.forEach((tpl) => {
    const name = tpl.name || 'Untitled';
    options.push(`<option value="${tpl.id}">${name}</option>`);
    dataOpts.push(`<option value="${name}"></option>`);
  });
  select.innerHTML = options.join('');
  if (datalist) {
    datalist.innerHTML = dataOpts.join('');
  }
}
...
async function handleSequenceStepEditSave(event) {
  ...
  const isEmail = step.step_type === 'email';
  const templateId = isEmail ? templateSelect.value : null;
  if (isEmail && !templateId) {
...
async function handleSequenceEnroll() {
  ...
  const contact =
    ...
    state.contacts.find((c) => buildContactLabel(c) === userEntry);
  if (!contact) {
